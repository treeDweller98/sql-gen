...
class VerdictReasoner(TextToSQL):
    ...
    def generate_prompt(self, schema: str, question: str, coder_outputs: list[str]) -> str:
        ...
        return (
            ...
            f"{responses}"
            "### BEST RESPONSE\n"
            "Select the most optimal SQL query based on the criteria above. Provide reasoning for your choice before presenting the final SQL query.\n\n"
            "Let's think step by step.\n\n"
        )
...
class ReasonerPicker:
    ...
    def run(
        ...
        output_path: Path, batch_size: int, savename: str, evaluator_fn
    ) -> pd.DataFrame:

        coder_1 = CoderAgent(llm, databases, output_path)
        coder_2 = CoderAgent(llm, databases, output_path)
        coder_3 = CoderAgent(llm, databases, output_path)
        reasoner = VerdictReasoner(llm, databases, output_path)
        ...
        coder_1_outputs, _ = coder_1.batched_generate(df, cfg, batch_size, 'coder1', None)
        coder_2_outputs, _ = coder_2.batched_generate(df, cfg, batch_size, 'coder2', None)
        coder_3_outputs, _ = coder_3.batched_generate(df, cfg, batch_size, 'coder3', None)
        ...
        best_outputs, best_labels = reasoner.batched_generate(
            df, cfg, batch_size, 'reasoner', evaluator_fn, coder_outputs=collect_coder_responses()
        )
