...
from sqlgen.plan import CoderAgent, MultiPlanCoderAgent, PlannerAgent
from sqlgen.zeroshot import ReasonerZeroShot, ZeroShotAgent
from utils import del_llm, load_llm
...
def zeroshot_experiment(
    ...
    outputs, labels = agent_zs.batched_generate(df, cfg, args.BATCH_SIZE, args.OUTPUT_DIR, args.EXPERIMENT, evaluate)
    del_llm(llm)
...
def reasoner_zeroshot_experiment(
    ...
    outputs, labels = agent_rzs.batched_generate(df, cfg, args.BATCH_SIZE, args.OUTPUT_DIR, args.EXPERIMENT, evaluate)
    del_llm(llm)
...
def planner_plan_experiment(
    ...
    outputs, labels = agent_plan.batched_generate(df, cfg, args.BATCH_SIZE, args.OUTPUT_DIR, args.EXPERIMENT, None)
    del_llm(llm)
...
def planner_exec_experiment(
    ...
    def compile_plans() -> pd.DataFrame:
        base_dir = Path(f"results/results_{args.DATASET}/plan")
        data = {}
    ...
    def combine_best_model_plans() -> pd.Series:
        ...
        return selected_plans_df.apply(
            lambda row: "\n\n".join(
                f"###### Plan {i + 1}\n{row[col].replace('### ', '')}"
                for i, col in enumerate(selected_plans_df.columns)
            ),
            axis=1,
        )
    ...
    for model in plan_df.columns:
        _, _ = plan_exec_agent.batched_generate(
            ...
            args.BATCH_SIZE,
            args.OUTPUT_DIR,
            f"{model}_solo_{args.EXPERIMENT}",
            evaluate,
            plans=plan_df[model],
            enable_zscot=enable_zscot,
        )
...
def reasoner_picker_experiment(
    ...
    def compile_zeroshot_outputs() -> dict[str, pd.Series]:
        ...
        matched_dirs = [
            d
            for d in Path(f"results/results_{args.DATASET}/zs").iterdir()
            if d.is_dir() and any(d.name.startswith(prefix) for prefix in ["gemma3", "qwen25", "qwen25_coder"])
        ]
