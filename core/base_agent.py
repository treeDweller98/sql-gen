...
def batched(sequence: Sequence[Any], n: int = 1):
...
class TextToSQLGenerationOutput:
    ...
    def as_dataframe(self, col_suffix: str | None = "") -> pd.DataFrame:
        if col_suffix:
    ...
    def __str__(self):
        ...
        return (
            f"Input Prompts: {self.input_prompts}\n"
            f"Raw Responses: {self.raw_responses}\n"
            f"Parsed SQL: {self.parsed_sql}\n"
            f"Total Input Tokens = {sum(self.n_in_tokens)}\n"
            f"Total Output Tokens = {sum(self.n_out_tokens)}"
        )
...
class TextToSQL(ABC):
    ...
    def process_question(self, idx: int, row: pd.DataFrame, **kwargs):
    ...
    def generate_text(
        self, prompts: list[str] | list[list[dict]], cfg: SamplingParams, use_tqdm: bool = False
    ...
    def batched_generate(
        ...
        **kwargs,
    ) -> tuple[TextToSQLGenerationOutput, list[bool] | None]:
        ...
        for i, batch in enumerate(tqdm(batched(df, batch_size), desc=f"{savename} Generating SQL")):
            # Generate
            prompts: list[str] = [
                self.generate_prompt(*self.process_question(idx, row, **kwargs)) for idx, row in batch.iterrows()
            ]
