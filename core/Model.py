...
class SupportedModels:
    ...
    class Ollama(Enum):
        ...
        granite_code_instruct_8b   = 'granite-code:8b-instruct-q5_K_M'
        granite_code_instruct_20b  = 'granite-code:20b-instruct-q5_K_M'
        granite_code_instruct_34b  = 'granite-code:34b-instruct-q5_K_M'
        starcoder2_instruct_15b    = 'starcoder2:15b-instruct-v0.1-q5_K_M'
        qwen_25_coder_instruct_7b  = 'qwen2.5-coder:7b-instruct-q5_K_M'
        qwen_25_coder_instruct_14b = 'qwen2.5-coder:14b-instruct-q5_K_M'
        qwen_25_coder_instruct_32b = 'qwen2.5-coder:32b-instruct-q5_K_M'
...
class LLM:
    ...
    def __init__(
        self, model: SupportedModels.Ollama | SupportedModels.OpenAI,
        api_key: Optional[str] = None, base_url: Optional[str] = None, 
        auth: Optional[HTTPBasicAuth] = None, timeout: float = 500.0,
        is_debug: bool = False,
    ):
        """ Attributes
            ----------
                ...
                client: openai.OpenAI | ollama.Client 
                    ...
                    Initialised using:
                        ...
                        auth: HTTPBasicAuth
                            used to authenticate ollama connection to base_url
                        timeout: float
                            client connection (and consequently model generation) timeout in seconds.
        ...
        self.client, self.__text_generator_fn = self.__setup_client(base_url, api_key, timeout, auth)
    ...
    def __setup_client(self, base_url: str, api_key: str, timeout: float, auth: HTTPBasicAuth) -> openai.OpenAI | ollama.Client:
        ...
        if self.model in SupportedModels.Ollama:
            ...
            client = ollama.Client(host=OLLAMA_BASE_URL, timeout=timeout, auth=auth)
            text_generator_fn = self.__generate_ollama

            try:
                LLM.__pull_ollama_model(client, self.model)
                print(f"Client Instantiated: Ollama model {self.model.value} active at {OLLAMA_BASE_URL}")
            except ollama.ResponseError as e:
                raise ValueError(f"Ollama pull() Failed: ({e.__class__.__name__} {e}).")
            except Exception as e:
                raise ValueError(
                    f"Ollama connection error: ({e.__class__.__name__} {e}). "
                    f"Ensure Ollama is running before instantiating LLM"
                )
